...
import { topUp } from '~/services/transactions/balances';
import { useDispatch, useSelector } from 'react-redux';
import { setBalance } from '~/providers/walletSlice';
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-4">
                ...
                <button onClick={handleTopup} disabled={!rawAmount || loading} className="w-full py-3 bg-[#f42619] text-white font-semibold rounded-md disabled:bg-gray-300 transition-colors">
            ...
            <div className="grid grid-cols-3 gap-3">
                {AMOUNT_OPTIONS.map((option) => (
                    <button key={option} type="button" onClick={() => setRawAmount(option.toString())} className="py-3 border border-gray-300 rounded-md text-sm text-gray-700 hover:bg-gray-100 transition-colors cursor-pointer">
